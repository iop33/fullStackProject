const {sequelize,Apoteka,Users}=require("../models");
...
rtr.get("/:id",(req,res)=>{
    ...
    Apoteka.findOne({where: {id:req.params.id}})
    .then(rows=>res.json(rows))
    .catch(err=>res.status(500).json(err));
});
rtr.post("/",(req,res)=>{
    ...
    const prov = Joi.object().keys({
        ...
        adresa: Joi.string().min(3).max(15).required(),
        locationsId: Joi.number().required(),
...
rtr.put("/:id",(req,res)=>{
    ...
    if(Validation.error){
        ...
        Apoteka.findOne({where: {id:req.params.id}})
        .then(apoteka=>{
            apoteka.ime= req.body.ime,
            apoteka.adresa= req.body.adresa,
            apoteka.locationsId= req.body.locationsId,
            apoteka.save()
        })
        .then(rows=>res.json(rows))
        .catch(err=>res.status(500).json(err));
