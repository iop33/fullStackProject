const {sequelize,Pozicija,Users}=require("../models");
...
rtr.get("/:id",(req,res)=>{
    ...
    Pozicija.findOne({where: {id:req.params.id}})
    .then(rows=>res.json(rows))
    .catch(err=>res.status(500).json(err));
});
rtr.post("/",(req,res)=>{
    ...
    const prov = Joi.object().keys({
        ime_pozicije:Joi.string().min(3).max(15).required(),
     });
    ...
    if(Validation.error){
        ...
        Pozicija.create({
            ime_pozicije: req.body.ime_pozicije,
...
rtr.put("/:id",(req,res)=>{
    ...
    if(Validation.error){
        ...
        Pozicija.findOne({where: {id:req.params.id}})
        .then(pozicija=>{
            pozicija.ime_pozicije= req.body.ime_pozicije,
            pozicija.save()
        })
        .then(rows=>res.json(rows))
        .catch(err=>res.status(500).json(err));
